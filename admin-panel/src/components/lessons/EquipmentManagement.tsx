...
export default function MaterialInventory() {
    ...
    try {
      ...
      if (equipmentResponse?.ok) {
        ...
        if (equipmentData.data && Array.isArray(equipmentData.data)) {
          equipmentList = equipmentData.data
      ...
      if (guestsResponse?.ok) {
        ...
        console.log('Guests API response:', guestsData)
        ...
        if (guestsData.data && Array.isArray(guestsData.data)) {
          ...
          guestsList = guestsData
        }

        console.log('Processed guests list:', guestsList)
        setGuests(guestsList.filter((guest: any) => guest.is_active))
      } else {
  ...
  const updateSelectedEquipmentData = async () => {
    ...
    try {
      ...
      if (response?.ok) {
        ...
        const updatedEquipment = equipmentList.find((equipment: Equipment) => equipment.id === selectedEquipment.id)
        if (updatedEquipment) {
    ...
    try {
      ...
      if (modalMode === 'create') {
        if (!equipmentFormData.name || !equipmentFormData.items || parseInt(equipmentFormData.items) < 1) {
          error('Bitte füllen Sie alle Pflichtfelder aus (Name und Anzahl Items)')
          return
        }
      }
      ...
      if (modalMode === 'edit' && selectedEquipment) {
        ...
        if (failedUpdates.length > 0) {
          error(`Error updating some equipment items`)
          return
        }
        ...
        if (!response?.ok) {
          ...
          error(`Error creating equipment: ${error.error || 'Unknown error'}`)
          return
        }
      ...
      console.error('Error:', err)
      error(`Error saving equipment: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  const handleDeleteEquipment = async (equipmentItem: Equipment) => {
    ...
    try {
      ...
      if (failedDeletions.length > 0) {
        error(`Error deleting some equipment items`)
        return
      }

      await loadData()
      error(`All ${itemsToDelete.length} items of "${baseName}" deleted successfully`)
    } catch (err) {
      console.error('Error:', err)
  ...
  const handleSaveAddUnit = async () => {
    ...
    try {
      ...
      if (!response?.ok) {
        const error = await response.json()
        error(`Error adding units: ${error.error || 'Unknown error'}`)
        return
      }

      await loadData()
      setShowAddUnitModal(false)
      error(`Successfully added ${addUnitFormData.count} units to ${baseName}`)
    } catch (err) {
      console.error('Error:', err)
      error('Error adding units')
    }
  }
  ...
  const handleDeleteUnit = async (equipment: Equipment) => {
    ...
    try {
      ...
      await loadData()
      success('Equipment unit deleted successfully')
    } catch (err) {
      console.error('Error:', err)
      error('Error deleting equipment')
    }
  ...
  const handleAssignEquipmentToGuest = async () => {
    ...
    if (!selectedEquipment.is_active) {
      error('Cannot assign guest to inactive equipment')
    ...
    if (guest) {
      ...
      if (existingAssignment) {
        ...
        if (existingEquipment && existingEquipment.category === selectedEquipment.category) {
          error(`Guest ${guest.name} already has ${selectedEquipment.category} assigned. A guest can only have one item per category.`)
          return
        }
      }
    }
    ...
    try {
      ...
      console.error('Error:', err)
      error('Error assigning equipment')
    }
  }
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              <div className="mb-2 p-2 bg-gray-100 rounded text-xs">
                <div>Debug: {guests.length} guests loaded</div>
                <div>Active guests: {guests.filter(g => g.is_active).length}</div>
              </div>
