...
export default function StaffOverviewComponent({ selectedDate, onDateChange }: StaffOverviewProps) {
  ...
  const updateMealCount = async (mealId: string, option: string, change: number) => {
    ...
    try {
      console.log('Updating meal portions:', { mealId, option, change, newCount, totalActualPortions })
      ...
      if (!response.ok) {
        ...
        console.error('Failed to update meal portions in database:', response.status, errorData)
      ...
      const result = await response.json()
      console.log('Successfully updated meal portions:', result)
    } catch (error) {
