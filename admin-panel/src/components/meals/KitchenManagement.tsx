...
export default function KitchenManagementComponent({ 
  ...
  useEffect(() => {
    const cutoffCheckInterval = setInterval(async () => {
      try {
        const response = await fetch('/api/meals/status').catch(() => null)
        if (response?.ok) {
  ...
  const loadMeals = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/meals').catch(() => null)
      if (response?.ok) {
        const result = await response.json()
        const meals = result.success ? result.data : result
        console.log('Raw meals from API:', meals)
        const convertedMeals = meals.map(convertMealToUI)
        console.log('Converted meals for UI:', convertedMeals)
  ...
  const loadTemplates = async () => {
    try {
      const response = await fetch('/api/meals?templates_only=true').catch(() => null)
      if (response?.ok) {
        const result = await response.json()
  ...
  const handleSaveAsTemplate = (meal: MealPlan) => {
    // Save meal as template to localStorage
    const template = {
    ...
    const existingTemplates = JSON.parse(localStorage.getItem('mealTemplates') || '[]')
    existingTemplates.push(template)
    localStorage.setItem('mealTemplates', JSON.stringify(existingTemplates))
    
    success('Meal saved as template successfully!')
  }
  ...
  const handleBulkDelete = async () => {
    if (selectedMealIds.length === 0) {
      error('Please select meals to delete')
      return
    }
    ...
    try {
      ...
      if (response?.ok) {
        const result = await response.json()
        error(`${result.deletedCount} meals deleted successfully!`)
        setSelectedMealIds([])
  ...
  const handlePublishMeal = async (mealId: string) => {
    try {
      ...
      if (response?.ok) {
        success('Meal published successfully!')
        loadMeals()
      } else {
        const error = await response.json()
        error(`Error: ${err.error}`)
      ...
      console.error('Error:', err)
      error('Error publishing meal')
    }
  }

  const handleUnpublishMeal = async (mealId: string) => {
    try {
      const response = await fetch('/api/meals', {
        method: 'PUT',
        headers: {
      ...
      if (response?.ok) {
        success('Meal unpublished successfully!')
        loadMeals()
      } else {
        const error = await response.json()
        error(`Error: ${err.error}`)
  ...
  const handleCopyMeal = async (meal: MealPlan) => {
    try {
      ...
      if (response?.ok) {
        success('Meal copied successfully!')
        loadMeals()
      } else {
        const error = await response.json()
        error(`Error: ${err.error}`)
                    ...
                    {categoryMeals.map((meal) => (
                      <div key={meal.id} className="relative">
                        ...
                        <UnifiedCard
                          id={meal.id}
                          title={meal.name}
                          subtitle={meal.dietary_option?.replace('_', ' ') || 'Meat'}
                          description={meal.description}
                          status={meal.status}
                          category={meal.category}
                          date={meal.scheduled_date}
                          time={`${meal.start_time} - ${meal.end_time}`}
                          participants={`${meal.planned_portions || 0} portions`}
                          color={meal.category === 'breakfast' ? '#F59E0B' : 
                                 meal.category === 'lunch' ? '#10B981' : '#8B5CF6'}
                          onView={(id) => handleViewMeal(meal)}
                          onEdit={(id) => handleEditMeal(meal)}
                          onPublish={(id) => handlePublishMeal(meal.id)}
                          onUnpublish={(id) => handleUnpublishMeal(meal.id)}
                          onCopy={(id) => handleCopyMeal(meal)}
                          onTemplate={(id) => handleSaveAsTemplate(meal)}
                          onDelete={(id) => handleDeleteMeal(meal.id)}
                        />
                      </div>
                    ))}
                  </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className="p-2 bg-red-600 text-white rounded-md hover:bg-red-700"
                title={`Delete ${selectedMealIds.length} Meals`}
              >
