...
export default function BedInventory() {
    ...
    try {
      ...
      if (modalMode === 'create') {
        if (!roomFormData.name || !roomFormData.bed_count || parseInt(roomFormData.bed_count) < 1) {
          alert('Bitte füllen Sie alle Pflichtfelder aus (Name und Anzahl Bed Items)')
          return
        }
      }
      ...
      alert(`Room ${modalMode === 'edit' ? 'updated' : 'created'} successfully`)
    } catch (error) {
  ...
  const handleAddBedToRoom = async (roomId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        alert(`Error adding bed: ${error.error}`)
        return
      }
  ...
  const handleRemoveBedFromRoom = async (bedId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        alert(`Error deleting bed: ${error.error}`)
        return
      }
  ...
  const handleViewBed = (bed: Bed) => {
    ...
    alert(`Viewing bed: ${bed.identifier} (${bed.bed_type})`)
  }
  ...
  const handleSaveBed = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        alert(`Error updating bed: ${error.error}`)
        return
      }
  ...
  const handleAssignBedToGuest = async () => {
    ...
    if (!selectedBed.is_active) {
      alert('Cannot assign guest to inactive bed')
    ...
    if (guest) {
      ...
      if (existingAssignment) {
        ...
        if (existingBed && existingBed.id !== selectedBed.id) {
          ...
          alert(`Guest ${guest.name} is already assigned to this bed.`)
          return
        }
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        alert(`Error assigning guest: ${error.error}`)
        return
      }
  ...
  const handleRemoveGuest = async (assignmentId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        alert(`Error removing guest: ${error.error}`)
        return
      }
  ...
  const handleBedNameChange = async (bedId: string, newName: string) => {
    try {
      ...
      console.error('Error updating bed name:', error)
      alert('Error updating bed name')
    }
  }
  ...
  const handleRoomNumberingTypeChange = async (newType: string) => {
    ...
    try {
      ...
      console.error('Error updating room numbering type:', error)
      alert('Error updating room numbering type')
    }
  }
  ...
  const handleRoomNumberingStartChange = async (newStart: string) => {
    ...
    try {
      ...
      console.error('Error updating room numbering start:', error)
      alert('Error updating room numbering start')
    }
  }
                    ...
                    return (
                      <div key={bed.id} className={`${cardBgColor} border ${cardBorderColor} rounded-lg p-4 hover:shadow-lg transition-all duration-200`}>
                        ...
                        <div className="mb-3 space-y-2">
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium text-gray-600">Kapazität:</span>
