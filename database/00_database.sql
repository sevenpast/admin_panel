-- =================================================================
-- CampFlow 2.0 - Complete Database Schema
-- Version: 1.4
-- Date: 2025-10-09
--
-- This single file contains the complete schema for the CampFlow 2.0
-- database. It is designed to be executed in a single run on a
-- clean Supabase (PostgreSQL) database.
-- =================================================================
...
CREATE TABLE beds (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    bed_id VARCHAR(12) UNIQUE NOT NULL,
    camp_id UUID NOT NULL REFERENCES camps(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_bed_assignments_guest_unique_active ON bed_assignments(guest_id) WHERE status = 'active';
...
BEGIN
    CASE prefix
        ...
        WHEN 'R' THEN
            table_name := 'rooms';
            id_column_name := 'room_id';
        WHEN 'B' THEN
            table_name := 'beds';
            id_column_name := 'bed_id';
        WHEN 'G' THEN
            table_name := 'guests';
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER set_bed_capacity_trigger BEFORE INSERT OR UPDATE OF bed_type ON beds FOR EACH ROW EXECUTE FUNCTION set_bed_capacity();

-- Trigger to update bed occupancy
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER update_bed_occupancy_trigger AFTER INSERT OR UPDATE OR DELETE ON bed_assignments FOR EACH ROW EXECUTE FUNCTION update_bed_occupancy();

-- Trigger to handle guest deactivation
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER guest_deactivation_trigger AFTER UPDATE OF is_active ON guests FOR EACH ROW EXECUTE FUNCTION handle_guest_deactivation();
...
-- Trigger to update job delivery stats
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER update_job_delivery_stats_trigger AFTER INSERT OR UPDATE OR DELETE ON automation_deliveries FOR EACH ROW EXECUTE FUNCTION update_job_delivery_stats();
...
CREATE POLICY rooms_camp_isolation ON rooms FOR ALL USING (camp_id IN (SELECT camp_id FROM user_sessions WHERE user_id = auth.uid() AND is_active = true));

ALTER TABLE beds ENABLE ROW LEVEL SECURITY;
CREATE POLICY beds_camp_isolation ON beds FOR ALL USING (camp_id IN (SELECT camp_id FROM user_sessions WHERE user_id = auth.uid() AND is_active = true));
...
CREATE POLICY bed_assignments_camp_isolation ON bed_assignments FOR ALL USING (camp_id IN (SELECT camp_id FROM user_sessions WHERE user_id = auth.uid() AND is_active = true));
